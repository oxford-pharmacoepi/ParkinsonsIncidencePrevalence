...
#                                                                            #
##############################################################################
info(logger, paste0('Making ', subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name), " into denominators"))
cdm <- generateDenominatorCohortSet(
  cdm = cdm,
  name = subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),
...
cdm <- generateDenominatorCohortSet(
  cdm = cdm,
  name = paste0(subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),"1y"),
  cohortDateRange = as.Date(c("2007-01-01", NA)),
  ageGroup = list(c(18,150),c(18, 30),c(31,40),c(41,50),c(51,60),c(61,70),c(71,80),c(81,150)),
  sex = c("Female", "Male", "Both"),
  daysPriorHistory = 365,
  strataTable = subtype_table_name_1y, 
  strataCohortId = subtypesCohortSet_1y %>% filter(cohort_name == subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name)) %>% pull(cohort_definition_id)
)
...
prevSubtype2 <- estimatePeriodPrevalence(cdm = cdm,
                                         denominatorTable = subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),
                                         outcomeTable = drug_table_name)

# 1.Plots for prevalence of the drug use in subtype 2
DrugsPrevalenceOverallSubtype2<- prevSubtype2 %>%
  ...
  ggtitle(paste0("Prevalence of antiparkinson drugs in people with ", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name))) +
...
DrugsPrevalenceOverallSubtype2Name <- paste0("DrugsPrevalence", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel), ".pdf")
...
# 2.Plots for prevalence of the drug use in subtype 2, stratified by age
DrugsPrevalenceBothSubtype2<- prevSubtype2 %>%
  ...
  labs(title = paste0("Prevalence of antiparkinson drugs in people with ", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name), ", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsPrevalenceBothSubtype2Name <- paste0("DrugsPrevalence", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"Both", ".pdf")
...
DrugsPrevalenceMaleSubtype2<- prevSubtype2 %>%
  ...
  facet_wrap(~denominator_age_group) +
  labs(title = paste0("Prevalence of antiparkinson drugs in men with ", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name), ", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsPrevalenceMaleSubtype2Name <- paste0("DrugsPrevalence", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel) ,"Male", ".pdf")
...
DrugsPrevalenceFemaleSubtype2<- prevSubtype2 %>%
  ...
  facet_wrap(~denominator_age_group) +
  labs(title = paste0("Prevalence of antiparkinson drugs in women with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsPrevalenceFemaleSubtype2Name <- paste0("DrugsPrevalence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"Female", ".pdf")
...
DrugsPrevalenceStratifiedByAgeAndSexSubtype2<- prevSubtype2 %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  mutate(denominator_age_group = replace(denominator_age_group, denominator_age_group == "18 to 150", "Over 18 years old")) %>%
  ...
  labs(title = paste0("Prevalence of antiparkinson drugs in people with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age and sex"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsPrevalenceStratifiedByAgeAndSexSubtype2Name <- paste0("DrugsPrevalence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"StratifiedByAgeAndSex", ".pdf")
...
incSubtype2 <- estimateIncidence(
  cdm = cdm,
  denominatorTable = paste0(subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),"1y"),
  outcomeTable = drug_table_name
)

# 1.Plots for incidence of the drug use in subtype 2
DrugsIncidenceOverallSubtype2<- incSubtype2 %>%
  filter(denominator_age_group == '18 to 150', denominator_sex == "Both") %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  ...
  ggtitle(paste0("Incidence of antiparkinson drugs in people with ", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name))) +
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsIncidenceOverallSubtype2Name <- paste0("DrugsIncidence", subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),".pdf")
...
DrugsIncidenceBothSubtype2<- incSubtype2 %>%
  filter(denominator_sex == "Both") %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  ...
  scale_y_continuous(labels = label_comma()) +
  facet_wrap(~denominator_age_group) +
  labs(title = paste0("Incidence of antiparkinson drugs in people with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsIncidenceBothSubtype2Name <- paste0("DrugsIncidence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"Both", ".pdf")
...
DrugsIncidenceMaleSubtype2<- incSubtype2 %>%
  filter(denominator_sex == "Male") %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  ...
  scale_y_continuous(labels = label_comma()) +
  facet_wrap(~denominator_age_group) +
  labs(title = paste0("Incidence of antiparkinson drugs in men with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsIncidenceMaleSubtype2Name <- paste0("DrugsIncidence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"Male", ".pdf")
...
DrugsIncidenceFemaleSubtype2<- incSubtype2 %>%
  filter(denominator_sex == "Female") %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  ...
  scale_y_continuous(labels = label_comma()) +
  facet_wrap(~denominator_age_group) +
  labs(title = paste0("Incidence of antiparkinson drugs in women with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsIncidenceFemaleSubtype2Name <- paste0("DrugsIncidence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"Female", ".pdf")
...
DrugsIncidenceStratifiedByAgeAndSexSubtype2<- incSubtype2 %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "COMTInhibitor", "COMT Inhibitors")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "DopamineAgonists", "Dopamine Agonists")) %>%
  mutate(outcome_cohort_name = replace(outcome_cohort_name, outcome_cohort_name == "MAOBInhibitors", "MAO-B Inhibitors")) %>%
  mutate(denominator_age_group = replace(denominator_age_group, denominator_age_group == "18 to 150", "Over 18 years old")) %>%
  ...
  scale_y_continuous(labels = label_comma()) +
  facet_grid(~denominator_sex ~denominator_age_group) +
  labs(title = paste0("Incidence of antiparkinson drugs in people with ",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name),", stratified by age and sex"))+
  labs(colour = "Parkinsonism Medications") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        panel.background = element_blank() ,
        axis.line = element_line(colour = "black", size = 0.6) ,
...
DrugsIncidenceStratifiedByAgeAndSexSubtype2Name <- paste0("DrugsIncidence",subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name_camel),"StratifiedByAgeAndSex", ".pdf")
...
#                                                                            #
##############################################################################
info(logger, paste0('Gathering results for antiparkinson drugs in ', subtypesCohortSet %>% filter(cohort_definition_id==2) %>% pull(cohort_name)))
exportIncidencePrevalenceResults(resultList = list("Prevalence of antiparkinson drugs" = prevSubtype2, "Incidence of antiparkinson drugs" = incSubtype2),
